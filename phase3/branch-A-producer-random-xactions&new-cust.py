...
class ProducerA:
    def __init__(self):
        self.producerA = KafkaProducer(bootstrap_servers=['localhost:9092'], value_serializer=lambda m: dumps(m).encode('ascii'))
    ...
    def generateRandomXactions(self, n=1000):
        for _ in range(n):
            ...
            if self.cursor.execute(f"select exists(select 1 from customer where custid={data['custid']});")==None:
                self.producerA.send("bank-customer-new", value={'custid':data['custid'],'fname':fake.first_name(),'lname':fake.last_name(),'createdate':time.time()})
            sleep(3)
                
            self.producerA.send('bank-customer-events-branch-A', value=data)
